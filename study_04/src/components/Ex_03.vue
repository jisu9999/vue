...
import { ref } from "vue";
//  탭목록 저장
const tabs = ref(["회사 소개", "서비스", "연락처", "예약 방법", "예약하기", "상담문의"]);
...
const submitReservation = () => {
  ...
  if (newReservation.value.service && newReservation.value.date && newReservation.value.contact) {
    // ...newReservation 기존 객체의 참조가 아니라 완전히 새로운 독립적인 객체를 추가하는 것! 기존 객체를 복사하여 새로운 객체를 생성
...
const toggleInquiry = () => {
  inquiryVisible.value = !inquiryVisible.value; // 토글기능(on:off)
};
</script>
<template>
  <div class="company-container">
    ...
    <div class="tab-content">
      ...
      <section v-if="activeTab === 4">
        ...
        <ul v-if="reservations.length">
          <li v-for="(res, index) in reservations" :key="index">
            <strong>{{ res.service }}</strong> - <strong>{{ res.date }}</strong> (📞{{ res.contact }})
          </li>
        </ul>
        <p>예약 내역이 없습니다.</p>
      </section>
      ...
      <div class="inquiry-modal" v-if="inquiryVisible" @click.self="toggleInquiry">
        <!-- @click.self하면 토글창 밖 아무대나 눌러도 닫기가 된다. -->
